...
function MapComponent(props){
      ...
      const fetchPrediction = async (city) => {
        try {
          const response = await fetch(`http://127.0.0.1:5000?city=${city}&time=${date}`);
          if (!response.ok) {
      ...
      // Fetch predictions for all cities on mount and when date changes
      useEffect(() => {
        const cities = [
          'Apex', 'Asheville', 'Burlington', 'Cary', 'Concord',
          'Chapel Hill', 'Charlotte', 'Durham', 'Fayetteville',
          'Greensboro', 'Gastonia', 'Greenville', 'High Point',
          'Huntersville', 'Jacksonville', 'Kannapolis', 'Raleigh',
          'Wake Forest', 'Wilmington', 'Winston-Salem'
        ];
    
        cities.forEach(fetchPrediction);
      }, [predictions]);
